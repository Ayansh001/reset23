-- Comprehensive OCR Integration Fix - Phase 1: Database Schema Corrections
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_ocr_jobs_status_user 
    ON public.ocr_jobs(status, user_id) WHERE status IN ('processing', 'pending');
    
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_ocr_jobs_file_status 
    ON public.ocr_jobs(file_id, status);
    
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_files_ocr_status_user 
